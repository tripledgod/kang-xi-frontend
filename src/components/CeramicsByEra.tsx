        ...
        <section className="w-full bg-[#F7F5EA] px-4 py-12">
            <div className="max-w-7xl mx-auto">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
                    <h2 className="text-4xl md:text-5xl font-serif font-medium mb-6 md:mb-0" style={{color: COLORS.secondary900}}>Ceramics by Era</h2>
                    <div className="hidden md:flex items-center gap-4 self-end md:self-auto">
                        <button
                            ...
                            disabled={!canGoLeft}
                        >
                            <img src={icLeft} alt="left" className="w-6 h-6" style={{display: "inline-block"}}/>
                        </button>
                        <button
                            className="w-10 h-10 border border-[#86684A] rounded-md items-center justify-center bg-transparent"
                            ...
                            disabled={!canGoRight}
                        >
                            <img src={icRight} alt="right" className="w-6 h-6" style={{display: "inline-block"}}/>
                        </button>
                        <button
                ...
                <div className="flex flex-col gap-8 mt-4 md:hidden">
                  {eras.map((era, idx) => (
                    <div key={era.name} className="flex flex-col bg-transparent">
                      ...
                      <div className="text-2xl font-serif mb-6 font-medium" style={{color: COLORS.primary900}}>{era.name}</div>
                      <div className="text-base font-semibold border-t border-[#C7C7B9] pt-4 text-[#2E2A24] mb-1">{era.years}</div>
                ...
                >
                  {eras.map((era, idx) => (
                      ...
                      >
                          <div
                              className="bg-[#E6DDC6] aspect-square w-full flex items-center justify-center overflow-hidden mb-4">
                              <img src={era.img} alt={era.name} className="object-cover w-full h-full"/>
                          </div>
